resource "digitalocean_droplet" "blue" {
  count = lookup(local.deploying_env[var.deployment_environment].blue, var.deployment_stage, 0) ? 1 : 0
  image = "ubuntu-20-04-x64"
  name = "blue-version-1.0-${count.index}"
  region = "fra1"
  size = "s-1vcpu-1gb"
  private_networking = true
  tags = ["console_v1_0", "blue_v1_0"]
  ssh_keys = [29902027]
  connection {
...
resource "cloudflare_record" "packetpigeon_blue" {
  name   = "packetpigeon.com"
  value  = digitalocean_droplet.blue[count.index].ipv4_address
  type = "A"
  proxied = false
